...
export function BatsmanBowlerPanel({ striker, nonStriker, bowler }: BatsmanBowlerPanelProps) {
  ...
  const renderBatsmanRow = (bat: BatsmanData, isStriker: boolean) => (
    <div className={clsx(
      ...
      isStriker ? "bg-emerald-500/[0.08]" : "hover:bg-white/[0.02]"
    )}>
      ...
      <div className="col-span-5 flex items-center gap-3 overflow-hidden relative z-10">
        {isStriker ? (
          <div className="relative flex items-center justify-center shrink-0">
            <span className="w-2 h-2 rounded-full border-2 border-emerald-400 bg-emerald-500 z-10" />
        ...
        <span className={clsx(
          ...
          isStriker ? "text-emerald-400" : "text-zinc-300"
        )}>
          {bat.name || 'Select Striker...'}
          {isStriker && <span className="ml-2 text-[10px] text-emerald-500/50 uppercase italic font-bold">*</span>}
        </span>
      </div>
      ...
      <div className="col-span-7 grid grid-cols-5 gap-1 text-right relative z-10 font-jetbrains">
        ...
        <div className="text-emerald-500/80 font-bold">{getSR(bat.runs, bat.balls)}</div>
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      {/* Batting Table */}
      <div className="bg-[#0e1424]/80 backdrop-blur-xl rounded-[1.25rem] border border-white/5 overflow-hidden shadow-2xl relative">
        <div className="absolute top-0 right-0 w-64 h-32 bg-emerald-500/5 blur-3xl pointer-events-none" />
        
        <div className="bg-white/[0.02] px-4 sm:px-6 py-3 border-b border-white/5 grid grid-cols-12 gap-2 text-[10px] sm:text-[11px] font-black uppercase tracking-widest text-zinc-500">
        ...
        {(striker.id || nonStriker.id) && (
          <div className="bg-black/40 px-6 py-3 border-t border-white/5">
            <div className="flex justify-between items-center mb-1.5 text-[10px] font-bold uppercase tracking-widest">
              <span className="text-emerald-500">{striker.runs}</span>
            ...
            <div className="h-1.5 w-full bg-zinc-800 rounded-full overflow-hidden flex">
              <motion.div 
                className="h-full bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.8)] rounded-l-full"
      ...
      <div className="bg-[#0e1424]/80 backdrop-blur-xl rounded-[1.25rem] border border-white/5 overflow-hidden shadow-2xl relative">
        ...
        <div className="grid grid-cols-12 gap-2 py-4 px-4 sm:px-6 items-center text-[13px] sm:text-sm font-medium hover:bg-white/[0.02] transition-colors relative z-10">
          ...
          <div className="col-span-7 grid grid-cols-5 gap-1 text-right font-jetbrains">
            ...
            <div className="text-blue-400/80 font-bold">{getEcon(bowler.runs, bowler.overs, bowler.balls)}</div>
          </div>
        </div>
      </div>
    </motion.div>
